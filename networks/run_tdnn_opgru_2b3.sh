...
# configs for TDNN-OPGRU networks
linear_layer_dim=64
layer_dim=256

# OPGRU training options
...
remove_egs=false
common_egs_dir=exp/chain/tdnn_opgru1b_sp/egs/
...
if [ $stage -le 12 ] && [ $stop_stage -gt 12 ] ; then
  ...
  norm-opgru-layer name=opgru1 cell-dim=$layer_dim recurrent-projection-dim=$linear_layer_dim non-recurrent-projection-dim=$linear_layer_dim delay=-3 dropout-per-frame=true dropout-proportion=0.0 $opts 
  ...
  norm-opgru-layer name=opgru2 cell-dim=$layer_dim recurrent-projection-dim=$linear_layer_dim non-recurrent-projection-dim=$linear_layer_dim delay=-3 dropout-per-frame=true dropout-proportion=0.0 $opts 
  relu-batchnorm-layer name=tdnn6 $opts  input=Append(-3,0,3) dim=$layer_dim
  relu-batchnorm-layer name=tdnn7 $opts  input=Append(-3,0,3) dim=$layer_dim
  norm-opgru-layer name=opgru3 cell-dim=$layer_dim recurrent-projection-dim=$linear_layer_dim non-recurrent-projection-dim=$linear_layer_dim delay=-3 dropout-per-frame=true dropout-proportion=0.0 $opts 

  ## adding the layers for chain branch
  output-layer name=output input=opgru3 include-log-softmax=false $output_opts

  output-layer name=output-xent input=opgru3 learning-rate-factor=$learning_rate_factor $output_opts
