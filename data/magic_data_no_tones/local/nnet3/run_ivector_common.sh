...
#     $num_utts ${temp_data_root}/${train_set}_sp_hires_subset
  utils/copy_data_dir.sh data/${train_set}_sp_hires ${temp_data_root}/${train_set}_sp_hires
  ...
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj $nj \
    ...
    ${temp_data_root}/${train_set}_sp_hires 512 \
...
if [ $stage -le 5 ] && [ $stop_stage -gt 5 ]  ; then
  ...
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj $nj --num-processes $num_processes \
