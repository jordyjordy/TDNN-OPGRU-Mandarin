...
pathToCorpus            = "/tudelft.net/staff-bulk/ewi/insy/SpeechLab/corpora/mandarin/Magic_Chinese_Mandarin_Conversational_Speech/md_cmn_conversational_speech/"
...
testSet = []
for entry in fs:
    if int(entry.name[-19:-16]) in range(0,27):
        trainSet.append(entry.name)
    if int(entry.name[-19:-16]) in range(27,31):
...
def main_func(dataFolder, dataSet):
    ...
    for entry in dataSet:
        ...
        f_TXT_Grid = open(pathToCorpus_TXT_Grid, "rt")
        ...
        for item in res:
            ...
            # TEXT
            f_text      = open(dataFolder+"text", "a")
            f_text.write(utt_id+" "+" ".join(item[4])+"\n")
            f_text.close()

            # UTT2SPK MDT_Conversation_001_SPK001-5.90-8.00 SPK001
            f_utt2spk = open(dataFolder+"utt2spk", "a")
